# Data Model: Unified Cloud Storage, Authentication & Licensing
...
This document defines the PostgreSQL schema for Sanyam IDE cloud storage, authentication, and licensing. The schema is designed for Supabase with Row-Level Security (RLS) policies for defense-in-depth.

## Entity Relationship Diagram

```
┌─────────────────┐       ┌─────────────────┐
│  user_profiles  │       │   tier_limits   │
│─────────────────│       │─────────────────│
│ id (FK auth)    │──────>│ tier (PK)       │
│ tier            │       │ max_documents   │
│ email           │       │ max_storage_*   │
│ display_name    │       │ has_* flags     │
│ storage_used    │       └─────────────────┘
└────────┬────────┘
         │
         │ owner_id
...
```sql
-- Subscription tiers
-- NOTE: Extensible via ALTER TYPE subscription_tier ADD VALUE 'trial'
-- Downstream code MUST handle unknown tiers by defaulting to 'free' limits
...
Single source of truth for tier capabilities and numeric limits. Changes take effect without code redeployment.
...
-- No RLS on tier_limits (public read, admin write)
```
...
CREATE TABLE documents (
  ...
  -- Content (text for DSL source, JSONB for structured data)
...
### Enforce Version Count Limits

```sql
-- Run after document save: FIFO deletion of excess versions
...
These interfaces are defined in `@sanyam/types`:
...
export interface TierLimits {
  ...
  has_cloud_auth: boolean;
  has_document_sharing: boolean;
  has_document_versioning: boolean;
  has_api_keys: boolean;
  has_realtime_collaboration: boolean;
  has_azure_ad: boolean;
}
...
The complete migration will be in `supabase/migrations/001_documents.sql` combining all the above schemas.
