    ...
    onStart(_app: FrontendApplication): MaybePromise<void> {
        ...
        this.desktopFileService.isInitialized().then(async initialized => {
            if (!initialized) {
                const messageContainer = document.createElement('div');
                // eslint-disable-next-line max-len
                messageContainer.textContent = nls.localizeByDefault('Would you like to create a .desktop file for the Sanyam IDE?\nThis will make it easier to open the Sanyam IDE directly\nfrom your applications menu and enables further features.');
                messageContainer.setAttribute('style', 'white-space: pre-line');
                const dialog = new ConfirmDialog({
