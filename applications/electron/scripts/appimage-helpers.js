...
function readPluginCopyMetadata(metadataPath) {
  ...
  try {
    ...
    console.warn(
      "Could not read built-in plugin copy metadata file:",
      err.message
    );
    return undefined;
  }
 ...
 * @param currentVersion - Current Sanyam IDE version
...
function copyBundledPlugins(bundledPluginsDir, userPluginsDir, currentVersion) {
  ...
  if (!metadata) {
    ...
    console.log(
      `Sanyam IDE updated from ${metadata.version} to ${currentVersion}. Updating built-in plugins...`
    );
    shouldCopy = true;
  }
  ...
  try {
    ...
    const pluginEntries = fs.readdirSync(bundledPluginsDir, {
      withFileTypes: true,
    });
