...
export class TheiaDesktopFileServiceEndpoint implements BackendApplicationContribution {
    ...
    protected async createOrUpdateDesktopfile(request: Request, response: Response): Promise<void> {
        ...
        if (createOrUpdate) {
            const imagePath = path.join(process.env.HOME!, '.local', 'share', 'applications', 'sanyam-ide-electron-app.png');
            if (!fs.existsSync(imagePath)) {
                const appDir = process.env.APPDIR;
                if (appDir !== undefined) {
                    const unpackedImagePath = path.join(appDir, 'sanyam-ide-electron-app.png');
